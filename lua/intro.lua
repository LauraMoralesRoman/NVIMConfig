...
local function show_centered_intro()
  ...
  local tot_h = vim.o.lines - vim.o.cmdheight - 1 -- total editor rows minus command‑line :contentReference[oaicite:2]{index=2}
  local tot_w = vim.o.columns -- total editor columns :contentReference[oaicite:3]{index=3}
  ...
  float_win = vim.api.nvim_open_win(float_buf, false, {
    ...
    vim.api.nvim_set_hl(0, 'FloatBackground', {
      bg = '#242424',  -- Your background color
      fg = '#cccccc',  -- Text color (optional)
    })

  -- 8. Apply per-window transparency & highlight
  vim.api.nvim_win_set_option(float_win, 'winhighlight', 'Normal:FloatBackground') -- override only this float’s highlights
  ...
  local ns = vim.api.nvim_create_namespace 'catnvim_intro'
  vim.api.nvim_buf_add_highlight(float_buf, ns, 'Pink', 1, 10, 19)
...
vim.api.nvim_create_autocmd('BufReadPost', {
  callback = function()
    ...
    if float_win and vim.api.nvim_win_is_valid(float_win) and name ~= '' then
      vim.api.nvim_win_close(float_win, true)
      float_win, float_buf = nil, nil
    end
  end,
