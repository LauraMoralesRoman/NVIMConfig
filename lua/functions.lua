...
vim.api.nvim_create_user_command('ToggleEditorGuides', function()
  ...
  if enabled then
    ...
    vim.bo.formatoptions = vim.bo.formatoptions .. 't' -- add 't' to auto-wrap as you type :contentReference[oaicite:2]{index=2}
  else
...
vim.api.nvim_create_user_command('AstGrep', function()
  ...
  require('fzf-lua').fzf_live('ast-grep run --pattern <pattern> .', {}, opts)
end, {
  nargs = 0,
...
vim.api.nvim_create_user_command('DeleteEmbeddingEntry', function(opts)
  ...
  local query =
    string.format("SELECT embeddings.id, content FROM embeddings JOIN collections ON collection_id = collections.id WHERE collections.name = '%s'", collection)
...
local function right_align()
  ...
  -- get character‑wise visual selection start/end (1‑based)
  local s = vim.fn.getpos "'<"
  local e = vim.fn.getpos "'>"
  local rs, cs = s[2] - 1, s[3] - 1 -- make 0‑based
  local re, ce = e[2] - 1, e[3] -- end_col is already exclusive
  ...
  -- grab _all_ text on that line, split into before/sel/after
  local line = vim.api.nvim_buf_get_lines(buf, rs, rs + 1, false)[1]
  local parts = vim.api.nvim_buf_get_text(buf, rs, cs, re, ce, {})

  -- parts is a list of lines; for single-line sel, parts[1] is exactly the selection
  local selection = parts[1]
  -- before = from column 0 up to cs
  local before = line:sub(1, vim.str_byteindex(line, cs))
  -- after  = from ce to end‑of‑line
  local after = line:sub(vim.str_byteindex(line, ce) + 1)

  -- determine target width
  local tw = vim.api.nvim_buf_get_option(buf, 'textwidth')
  if tw == 0 then
    tw = vim.o.columns
  end

  local rem_width = vim.fn.strdisplaywidth(before .. after)
  local sel_width = vim.fn.strdisplaywidth(selection)
  local pad = tw - rem_width - sel_width
  if pad < 1 then
    pad = 1
  end

  -- rebuild the line
  local newline = before .. after .. string.rep(' ', pad) .. selection
  vim.api.nvim_buf_set_lines(buf, rs, rs + 1, false, { newline })
end

vim.api.nvim_create_user_command('Right', function(opts)
  -- set the '< and '> marks to the given range
  vim.fn.setpos("'<", { 0, opts.line1, 1, 0 })
  vim.fn.setpos("'>", { 0, opts.line2, 1, 0 })
  right_align()
